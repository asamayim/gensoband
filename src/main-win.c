...
#define IDM_OPTIONS_MUSIC		  411 //v1.1.58
...
static cptr basic_music_file[MUSIC_BASIC_MAX][SAMPLE_MUSIC_MAX];
static cptr dungeon_music_file[DUNGEON_MAX][SAMPLE_MUSIC_MAX];
static cptr town_music_file[TOWN_MAX][SAMPLE_MUSIC_MAX];
static cptr quest_music_file[QUEST_MAX][SAMPLE_MUSIC_MAX];

static cptr low_mon_music_file[MON_IDX_MAX][SAMPLE_MUSIC_MAX];
static cptr med_mon_music_file[MON_IDX_MAX][SAMPLE_MUSIC_MAX];
static cptr high_mon_music_file[MON_IDX_MAX][SAMPLE_MUSIC_MAX];
...
//v1.1.79 ウィンドウサイズがうまく保存されない問題を解決するために本家コードをコピー
#if TRUE
...
static s16b tokenize_whitespace(char *buf, s16b num, char **tokens)
{
	...
	while (k < num)
	{
		...
		//v1.1.58 <>で囲まれた文字列は半角スペースで区切らないことにした
		if (*s == '<')
		{
			...
			if (*t) *t++ = '\0'; //　>のあった位置に代わりに\0を入れ、次の文字列取得のためにポインタを一文字進める
...
static void load_music_prefs(void)
{
	...
	{
		GetPrivateProfileString("Basic", angband_music_basic_name[i], "", tmp, 4096, ini_path);
		...
		for (j = 0; j < num; j++)
		{
			...
			/* Access the sound */
			//文字列に:が含まれていたらフルパスとみなす
			if (strchr(zz[j], (int)':'))
				(void)strnfmt(wav_path, 1024, "%s", zz[j]);
			else
				path_build(wav_path, 1024, ANGBAND_DIR_XTRA_MUSIC, zz[j]);

			/* Save the sound filename, if it exists */
			if (check_file(wav_path))
				basic_music_file[i][j] = string_make(zz[j]);
			else
				msg_format("WARNING:音楽ファイル[%s]のパスが確認できない", wav_path);

			/*:::パラメータが登録されていない場所は初期化しなくていいのか？と思ったが、グローバル変数のポインタ型は自動的にNULLで初期化されるらしい。知らんかった*/
		}
	}
	...
	for (i = 0; i < DUNGEON_MAX; i++)
	{
		...
		GetPrivateProfileString("Dungeon", key, "", tmp, 4096, ini_path);
	...
	for (i = 0; i < QUEST_MAX; i++) 
	{
		...
		GetPrivateProfileString("Quest", key, "", tmp, 4096, ini_path);
	...
	for (i = 0; i < TOWN_MAX; i++) 
	{
		...
		GetPrivateProfileString("Town", key, "", tmp, 4096, ini_path);
	...
	//[Monster_Low][Monster_Med][Monster_High]セクションを読み取る。
	//r_idxの数だけ読み取りしてると時間がかかるのでキー一覧を取得してからそのキーごとに行う。
	for (pr = MON_MUSIC_PRIOR_LOW; pr <= MON_MUSIC_PRIOR_HIGH; pr++)
	{
		char buf_mon_keys[32767];
		char *p;
		int key_list_length;

		//キー一覧とその文字列長を取得
		if (pr == MON_MUSIC_PRIOR_LOW)
			key_list_length = GetPrivateProfileString("Monster_Low", NULL, "", buf_mon_keys, 32767, ini_path);
		else if (pr == MON_MUSIC_PRIOR_MED)
			key_list_length = GetPrivateProfileString("Monster_Med", NULL, "", buf_mon_keys, 32767, ini_path);
		else
			key_list_length = GetPrivateProfileString("Monster_High", NULL, "", buf_mon_keys, 32767, ini_path);

		//キー一覧が格納された文字列(\0で区切られている)の終わりまでループ
		p = buf_mon_keys;
		while (key_list_length > 0)
		{
			int tmp_len = strlen(p) + 1;
			i = 0;

			//pから\0までをキーとしてパラメータを取得
			if (pr == MON_MUSIC_PRIOR_LOW)
				GetPrivateProfileString("Monster_Low", p, "", tmp, 4096, ini_path);
			else if (pr == MON_MUSIC_PRIOR_MED)
				GetPrivateProfileString("Monster_Med", p, "", tmp, 4096, ini_path);
			else
				GetPrivateProfileString("Monster_High", p, "", tmp, 4096, ini_path);

			sscanf(p,"mon%d",&i);

			if (i < 1 || i >= MON_IDX_MAX)
			{
				msg_format("WARNING:不正なキー[%s]", p);
			}
			//パラメータを取得できたらそれをパースしファイルパス名に整形しチェックし配列に格納
			else
			{
...
//						msg_format("i:%d j:%d prior:%d title:%s", i, j, pr, wav_path);

						if (pr == MON_MUSIC_PRIOR_LOW)
							low_mon_music_file[i][j] = string_make(zz[j]);
						else if (pr == MON_MUSIC_PRIOR_MED)
							med_mon_music_file[i][j] = string_make(zz[j]);
						else
							high_mon_music_file[i][j] = string_make(zz[j]);
					}
					else
						msg_format("WARNING:音楽ファイル[%s]のパスが確認できない", wav_path);
				}
			...
			//次のキーまでポインタを進める。
			//GetPrivateProfileStringでキー一覧を取得したときはキーが\0で分割されていて最後は\0\0で終わる。
			p += tmp_len;
			if (!*p) break;
			//...はずだがどうも自信がないのでGetPrivateProfileString()戻り値の文字列長もループ脱出条件に加えとく。
			key_list_length -= tmp_len;

		}
	}

	//オプション項目を読み取る
	GetPrivateProfileString("Options", "UniqueClassUseMonsterBgm", "", tmp, 1024, ini_path);
	if (strcmp(tmp, "ON") == 0) flag_unique_class_use_monster_bgm = TRUE;

	GetPrivateProfileString("Options", "DisplayBgmTitle", "", tmp, 1024, ini_path);
	if (strcmp(tmp, "ON") == 0) flag_display_bgm_title = TRUE;

}


//v1.1.58
//r_idxから、music.cfgのどの優先度に登録されているかを確認する
int find_mon_music_priority(int r_idx)
{
	if (r_idx < 1 || r_idx >= MON_IDX_MAX) return MON_MUSIC_PRIOR_NONE;

	if (high_mon_music_file[r_idx][0]) return MON_MUSIC_PRIOR_HIGH;

	if (med_mon_music_file[r_idx][0]) return MON_MUSIC_PRIOR_MED;

	if (low_mon_music_file[r_idx][0]) return MON_MUSIC_PRIOR_LOW;
...
//v1.1.58 本家BGM機能をコピー
//init_music周りの処理をキャラメイク時に実行できるよう分割した
#ifdef USE_MUSIC
...
//BGMファイル名(フルパスのことがある)からフォルダ名と拡張子(ないことがある)を削除し、
//cptr bgm_title_textにコピーして画面に表示する
void display_bgm_title(cptr bgmfilename)
{
	char buf[1024];
	...
	if (!use_music || !flag_display_bgm_title) return;

	if (strlen(bgmfilename) > 1000)
	{
		msg_print("WARNING:ファイルパスが長すぎてBGMタイトルを表示できない");
		bgm_title_text_ptr = NULL;
		return;
	}
	else if(strlen(bgmfilename) < 1)
	{
		bgm_title_text_ptr = NULL;
		return;
	}

	my_strcpy(buf, bgmfilename, sizeof(buf));

	char_count = 0;
	//ファイル名を最初からループ
	for (p = buf; *p; p++)
	{
		//ファイル名の一文字目の場合ポインタの位置を記録
		if (!char_count)
		{
			filename_start = p;
			filename_end = p;
		...
		//ファイル名2文字目以降に.があったら拡張子を示すかもしれないので記録
		if (*p == '.' && char_count) filename_end = p;
	...
	//filename_endがスタート位置から変化していたら(ファイル名中最後に使われた.の位置)そこで文字列を終わらせるため\0にする
	if (filename_end > filename_start) *filename_end = '\0';

	//文字列から抽出したファイル名部分を領域確保して保存
	bgm_title_text_ptr = string_make(format("BGM.%s", filename_start));


	//画面が通常フロアでない場合直接表示する
	if (character_icky || character_xtra)
	{
		int wid, hgt, row, col;
		Term_get_size(&wid, &hgt);

		row = (hgt + ROW_STATBAR);
		col = wid - strlen(bgm_title_text_ptr) - 10;

		if (col > 20) c_put_str(TERM_WHITE, bgm_title_text_ptr, row, col);

	}

	//prt_status()からも表示するので再描画フラグ
...
/*:::v1.1.58 本家BGM機能をコピーして改変
 *指定した主分類、副分類のBGMを鳴らす。
 *演奏開始したとき、または同じ系統の音楽がすでに鳴っている時に0が帰る。注意。
 *n:主分類 TERM_XTRA_MUSIC_***
 *v:副分類あるいは街やダンジョンのidx
 *dungeon_music_file(主分類によって分岐)[(副分類)][(16個までどれかランダム)]のファイル名の音楽が鳴らされる。
 */
...
//	if (cheat_xtra) msg_format("play_music2(%d,%d)", n, v);
	...
	//n,vに合致する音楽ファイル名がいくつ記録されているか数えている
	switch (n)
	{
	case TERM_XTRA_MUSIC_BASIC:
		for (i = 0; i < SAMPLE_MUSIC_MAX; i++) if (!basic_music_file[v][i]) break;
		break;
	case TERM_XTRA_MUSIC_DUNGEON:
	...
	case TERM_XTRA_MUSIC_TOWN:
		...
		break;
	case TERM_XTRA_MUSIC_MON_L:
		for (i = 0; i < SAMPLE_MUSIC_MAX; i++) if (!low_mon_music_file[v][i]) break;
		break;
	case TERM_XTRA_MUSIC_MON_M:
		for (i = 0; i < SAMPLE_MUSIC_MAX; i++) if (!med_mon_music_file[v][i]) break;
		break;
	case TERM_XTRA_MUSIC_MON_H:
		for (i = 0; i < SAMPLE_MUSIC_MAX; i++) if (!high_mon_music_file[v][i]) break;
		break;
	}
	...
	//記録されたファイル名からランダムに1つ選ぶ
	//main-win内ではランダム値を取るのにRand_external()を使うらしい
	switch (n)
	{
	case TERM_XTRA_MUSIC_BASIC:
		music_name_ptr = basic_music_file[v][Rand_external(i)];
		break;
	case TERM_XTRA_MUSIC_DUNGEON:
		music_name_ptr = dungeon_music_file[v][Rand_external(i)];
		break;
	case TERM_XTRA_MUSIC_QUEST:
		music_name_ptr = quest_music_file[v][Rand_external(i)];
		break;
	case TERM_XTRA_MUSIC_TOWN:
		music_name_ptr = town_music_file[v][Rand_external(i)];
		break;
	case TERM_XTRA_MUSIC_MON_L:
		music_name_ptr = low_mon_music_file[v][Rand_external(i)];
		break;
	case TERM_XTRA_MUSIC_MON_M:
		music_name_ptr = med_mon_music_file[v][Rand_external(i)];
		break;
	case TERM_XTRA_MUSIC_MON_H:
		music_name_ptr = high_mon_music_file[v][Rand_external(i)];
		break;
	default:
		msg_print("ERROR:TERM_XTRA_MUSIC_???が渡された");
		return (1);
	}
	//ファイル名に:が含まれていたらフルパスと見なしてそのままファイル名にし、
	//そうでなければmusicフォルダのパスにファイル名をつなげる
	if (strchr(music_name_ptr, (int)':'))
		(void)strnfmt(buf, 1024, "%s", music_name_ptr);
	else
		path_build(buf, 1024, ANGBAND_DIR_XTRA_MUSIC, music_name_ptr);

	display_bgm_title(buf);
...
#ifdef WIN32
	//MCI APIという機能を使って音楽を鳴らしているらしい。理解を放棄し本家ルーチンを有難く丸コピーさせて頂く。
...
static errr Term_pict_win(int x, int y, int n, const byte *ap, const char *cp, const byte *tap, const char *tcp)
{
	...
	for (i = 0; i < n; i++, x2 += w2)
	{
		...
		char c = cp[i];
		/*:::memo
		 *graf-xxx.prf/graf-new.prfに"0xDC:0x83"のように記述されている値が8x8.bmp/16x16.bmpの中での位置を示している。
		 *ゲームを起動したとき最初の値がattr値に、次の値がchar値に格納されている。
		 *このそれぞれの値の最上位ビットを除去した値が画像ファイル内の左上から見た列/行(0から開始)になる。
		 *例えばgraf-xxx.prfの"R:1:0x9B:0x8A"は、最上位ビットを除き10進数にしたら27/10。
		 *8x8.bmpの上から28番目、左から11番目の位置にある画像がr_idx=1(汚いイタズラ小僧)のグラフィックになる。
		 */
		/* Extract picture */
		int row = (a & 0x7F);
		int col = (c & 0x7F);
...
static void init_windows(void)
{
	...
	//v1.1.13 起動時のウィンドウサイズが小さい場合少し大きくする。どこでウィンドウ最低サイズ決めてるのか探したけどわからなかった
	if (td->rows < 30)  td->rows = 30;
	if (td->cols < 100) td->cols = 100;
...
static void setup_menus(void)
{
	...
	CheckMenuItem(hm, IDM_OPTIONS_MUSIC,
		(arg_music ? MF_CHECKED : MF_UNCHECKED)); //v1.1.58
...
///\131117 sysdel
		...
		{
			///\131117 sysdel
			//	plog("ごめんなさいムービー機能とかよく分からんので消しました");
//#if 0
			if (!initialized)
			{
#ifdef JP
...
//#endif
		...
		{
			term_data *td2 = &data[0];

			///\131117 sysdel
			//plog("工事中");
//#if 0 //v1.1.55 全角描画を有効にする。
			//前試したときにはところどころで描画がおかしくなってフリーズした気がするんだが、なぜか今試したらなんともなかった。
			/* Paranoia */
			if (!inkey_flag)
			{
				plog("You may not do that right now.");
			...
			/* Resize the term */
			Term_resize(td2->cols, td2->rows);

			/* Redraw later */
			InvalidateRect(td2->w, NULL, TRUE);
//#endif
			break;
		}

		case IDM_OPTIONS_MUSIC: //v1.1.58
		{
			/* Paranoia */
			if (!inkey_flag)
			{
...
/*:::ウィンドウメニューとかの動作　たぶん通常のメニューのゲーム開始はここから*/
#ifdef __MWERKS__
		       ...
		       LPSTR lpCmdLine, int nCmdShow)
{
	...
	//v1.1.29 windows8.1以降でサブウィンドウONの状態で起動しようとするとフリーズする不具合の修正を本家からコピー
	Term_activate(term_screen);
	...
	/*:::このCHUUKEIに関する記述は全て削除する予定*/
#ifdef CHUUKEI
	/*:::コマンドラインで実行したときのオプション解釈？*/
	if(lpCmdLine[0] == '-'){
	  switch(lpCmdLine[1])
	  {
	  case 'p':
