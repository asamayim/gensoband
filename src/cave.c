...
{
	//v1.1.68  新地形「氷塊」 BASHフラグがあるが開けられないので強制FALSEにする
	//v1.1.69 FF_OPENのままにしてて隠しドアが見つからなくなっていたので修正
	if (cave_have_flag_grid(c_ptr, FF_ICE_WALL))
		return FALSE;
...
static bool check_local_illumination(int y, int x)
{
	...
	/*:::光源に照らされてる判定？　光源やライトエリアはmimicフィールドにCAVE_GLOWが立つのか？*/
 ...
 *:::CAVE_GLOWを持つ壁はそのまま処理すると全方向から見えて変なので、
 *:::CAVE_GLOWを持つ床と隣接しててその床に＠の視線が通っているときだけ処理するようにする？
 *
 ...
 *:::このチェックは処理能力を食う。ダンジョンの最外周の壁にこのチェックをやるとバグる？
 *
 *
 ...
 *:::バグ回避と効率化のために、「全てのCAVE_GLOWの永久壁は全方向から照らされてる」ものとして扱う。
 *vaultと街ではこの処理は不自然だが、街は別処理するしvaultではそんなこと気にしてる場合じゃない。
 *
 *
 ...
 *:::CAVE_GLOWの壁は照らされた床に隣接してないと処理されないので、部屋の外から壁だけ見えたりはしない。
 */
bool player_can_see_bold(int y, int x)
{
	...
	/*:::los判定の前にここが判定されているところを見ると、これらのフラグも＠が歩くたびに更新されているのか？*/
	...
	/*:::ニンジャは常に見える　吸血鬼の処理もここか？*/
	if (p_ptr->see_nocto) return TRUE;

	/* Require "perma-lite" of the grid */

	/*:::CAVE_GLOWとCAVE_MNDKのフラグを抽出し、それがCAVE_GLOWのみと一致しないとFALSEを返す。
	 *:::CAVE_GLOWだけONならそのまま先へ
	 *:::MNDKだけONならFALSEを返して終了
	 *:::両方ONならFALSEを返して終了
	 *:::両方OFFならFALSEを返して終了
	 *:::→もともと光ってる床、かつmonが暗くしていない床でないと＠の光源やライトエリアを無視して不可視処理・・？
	 *:::Q008これじゃ通常の床で光源が効かない。CAVE_GLOWやCAVE_LITEに関して誤解してるかこの処理を読み間違えているらしい。
	 *:::↑どうもライトエリアや光源処理で照らされた床もCAVE_GLOW扱いになっている？まだ詳細不明*/
	if ((c_ptr->info & (CAVE_GLOW | CAVE_MNDK)) != CAVE_GLOW) return FALSE;

	/* Feature code (applying "mimic" field) */
	/* Floors are simple */
	/*:::cave_type.mimicがあればmimicの値のf_info[].mimic、なければcave_type.featのf_info[].mimicを対象に、*/
	/*:::f_info[mimic].flagsにFF_LOSが立ってるか判定。立ってたらTRUE*/
	/*:::Q009この機能の意味がよく解らない*/
 ...
 *:::地形・アイテム・モンスターの順に処理。効率悪いがだいたい普通の地形なので問題なし
 *
 ...
 *:::多色、透明モンスターの処理もここでするがマルチシンボルの処理はしない・・？
 *
 ...
 *:::多色アイテムは存在しないので処理しない。作るならdungeon.cを弄る必要がある。
 *
 ...
 *:::未発見トラップは床として描画。special_liteの地形は光る・・？
 *
 ...
 *:::mimicフィールドがあると別の地形として描画される。隠しドアとか
 *
 ...
 *:::グリッドが見えるためには、照らされており視線が通っており＠が盲目でない事
 *
...
/*:::メイン画面の指定位置にどの文字記号を表示するのか計算　モンスター・アイテム・地形の順に優先　詳細未読*/
/*:::幻覚処理もここ*/
void map_info(int y, int x, byte *ap, char *cp, byte *tap, char *tcp)
{
	...
	if (!have_flag(f_ptr->flags, FF_REMEMBER))
	{
		     ...
		     ((c_ptr->info & CAVE_VIEW) && (((c_ptr->info & (CAVE_GLOW | CAVE_MNDK)) == CAVE_GLOW) || p_ptr->see_nocto))))
		{
			...
			///mod150818 月都万象展クエスト特殊処理
			else if(p_ptr->inside_quest == QUEST_MOON_VAULT && quest[QUEST_MOON_VAULT].flags & QUEST_FLAG_SPECIAL)
			{
				a = TERM_BLUE;
			}
			...
			//v1.1.35
			else if(c_ptr->cave_xtra_flag & CAVE_XTRA_FLAG_NEMUNO)
			{
				a = TERM_RED;
			}
	...
	else
	{
		...
		//v1.1.48 紫苑のスーパー貧乏神モード
		if (SUPER_SHION)
		{
			a = TERM_BLUE;
		}
		else if (YUMA_ULTIMATE_MODE)
		{
			a = TERM_RED;
		}
	}
	...
	if (c_ptr->m_idx && display_autopick == 0 )
	{
		...
		if (m_ptr->ml)
		{
			...
			/* Hallucination */
			///mod150328 ぬえの正体不明の種処理追加
			if (p_ptr->image ||  (m_ptr->mflag & MFLAG_NUE_UNKNOWN))
			{
				/*
			...
			else
			{
				...
				//マミゾウ特技で弱体化
				if(p_ptr->pclass == CLASS_MAMIZOU && p_ptr->magic_num1[0] == c_ptr->m_idx)
					c = 'x';
 ...
 */
/*:::フロアの壁やアイテム・トラップを記憶して床を記憶しないなどの処理*/
/*:::cave[][].infoにCAVE_MARKフラグを立てることで記憶する*/
void note_spot(int y, int x)
{
	...
	/*:::照らされてなく視線の通ってないマス、視界内だが暗くてノクトビジョンのない時は何もしない？*/
	...
	if (!(c_ptr->info & (CAVE_LITE | CAVE_MNLT)))
	{
		...
		/* Require "perma-lite" of the grid */
		/*:::「GLOWがTRUE、かつMNDKがFALSE」以外はここに入る*/
	...
	/*:::対象マスにアイテムが落ちていた場合o_list[].markedにOM_FOUNDフラグを追加*/
	for (this_o_idx = c_ptr->o_idx; this_o_idx; this_o_idx = next_o_idx)
	{
...
static void display_shortened_item_name(object_type *o_ptr, int y)
{
	...
	*c='\0';
	Term_putstr(0, y, 18, attr, buf);
}

/*
 * Display a "small-scale" map of the dungeon in the active Term
 */
/*:::ミニ全体マップを表示する。Mコマンドのほかサブウィンドウのマップ表示で使う*/
/*:::cy,cxはミニマップ内で＠の位置を返すのに使われる。Mコマンドのカーソル操作に使うらしい*/
...
void update_lite(void)
{
	...
	//v1.1.66 特定条件のグリッドをカウントするtemp_nをリセットしておく
	temp_n = 0;
	...
	/* None left */
	lite_n = 0;
//testmsg(format("py:%d px:%d",py,px));
//testmsg(format("wy:%d wx:%d",p_ptr->wilderness_y,p_ptr->wilderness_x));
...
void update_mon_lite(void)
{
	...
	s16b fx, fy;
	/*:::mon_lite_hack()かmon_dark_hack()になる*/
...
void update_view(void)
{
	...
	for (n = 0; n < view_n; n++)
	{
		...
		//v1.1.84 さとりは罠を見た瞬間発見できることにした
		if (!character_xtra && p_ptr->pclass == CLASS_SATORI && p_ptr->lev > 14 && (PLAYER_CAN_USE_EYES) && c_ptr->mimic && cave_have_flag_bold((y), (x), FF_TRAP))
		{
			disclose_grid(y, x);
			msg_print("トラップを発見した。");
			disturb(0, 1);
...
//v1.1.24 update_flow()を改変
/*:::cave[][].flying_distを計算する。
 * 各グリッドについて＠のいる地点からの浮遊状態での移動距離を求める。飛行モンスターが＠を追跡するのに使う。
 * 従来、モンスターは「＠から射線が通っておらず＠が通ったことのない(when値のない)暗い穴経由の通路を越えて＠を追うことはない」仕様。
 * 本家では竜窟のかなり限定された地形とモンスター配置でないと悪用できないので大した問題にはならなかったようだが、
 * 勝手版では魔法で暗い穴地形を生成できるので対策することにした。
 * 多少処理時間が増えるがよほど古いマシンでない限り問題ないだろう。
 */
void update_flying_dist(void)
{
	int x, y, d;
	int flow_head = 1;
	int flow_tail = 0;
	...
	while (flow_head != flow_tail)
	{
		...
		for (d = 0; d < 8; d++)
		{
			...
			int n = cave[ty][tx].flying_dist + 1;
			cave_type *c_ptr;
			...
			if (c_ptr->flying_dist != 0 && c_ptr->flying_dist <= n) continue;


			//v1.1.43 サニー特技用特殊フラグ処理。閉じたドアやカーテンを通らない。ガラスは通る。
			if (flag_sunny_refraction)
			{
				if(!cave_have_flag_grid(c_ptr, FF_LOS))		continue;
			}
			else
			{

				/*::: 壁や岩など移動不可地形を除く。ただし飛行侵入可能な「暗い穴」「山脈(地上の)」地形はOKとする。*/
				if (!cave_have_flag_grid(c_ptr, FF_MOVE) && !is_closed_door(c_ptr->feat) && !cave_have_flag_grid(c_ptr, FF_CAN_FLY)) continue;

			}


			if (c_ptr->flying_dist == 0 || c_ptr->flying_dist > n) c_ptr->flying_dist = n;
 ...
 */
/*:::cave[][].costを計算する。＠がそれぞれのマスに到達するのに掛かる手間の概算値・・？*/
...
void map_area(int range)
{
	...
	for (y = 1; y < cur_hgt - 1; y++)
	{
		for (x = 1; x < cur_wid - 1; x++)
		{
			...
			///mod150116 村紗の自動水地形感知
			if(murasa_detect_water && !have_flag(f_ptr->flags, FF_WATER)) continue;
...
void wiz_lite(bool ninja)
{
	...
	for (i = 1; i < o_max; i++)
	{
		...
		//v1.1.43 サニー特技専用フラグ 別の場所で計算したflying_dist値のあるところだけ啓蒙する
		if (flag_sunny_refraction)
		{
			if (!cave[o_ptr->iy][o_ptr->ix].flying_dist) continue;
		}
 ...
 */
/*:::地形変更。主に床を何か別のものにするとき*/
/*:::鏡･ルーンは消える*/
/*:::生成後ダンジョンに対する変更の場合、モンスターや光源関係の情報を更新する*/
void cave_set_feat(int y, int x, int feat)
{
	...
	/*:::ダンジョン生成中のときcharacter_dungeon=FALSE*/
	...
	/*:::ここから先は生成済みダンジョンに対する何らかの行動による変更*/
...
/*:::newfeat:f_infoの地形インデックス？入ってない場合はどうなる？*/
int conv_dungeon_feat(int newfeat)
{
	feature_type *f_ptr = &f_info[newfeat];

	/*:::defines.hにFF_????としてフラグが定義されている*/
	if (have_flag(f_ptr->flags, FF_CONVERT))
	{
		switch (f_ptr->subtype)
		{
		case CONVERT_TYPE_FLOOR:
			/*:::set_floor_and_wall()でダンジョン別に壁の種類が設定されているはず*/
 ...
 */
/*:::地形に対して取った特定の行動によりその地形がどう変わるかを決定。詳細未読*/
 ...
 */
/*:::地形変更。主に何かを壊して床にするとき。*/
/*:::新しい地形を決定し、cave_set_feat()で変更する*/
/*:::金やアイテムを掘り出したりガラス地形を壊す処理も行っている*/
void cave_alter_feat(int y, int x, int action)
{
	...
	if (!(feature_action_flags[action] & FAF_NO_DROP))
	{
		...
		///mod140405 岩石を崩したら岩が転がってくることにした
		/* Handle item */
		if (have_flag(old_f_ptr->flags, FF_HAS_ITEM) && !have_flag(f_ptr->flags, FF_HAS_ITEM) && (one_in_(2)))
		{
			int i;
			object_type forge;
			object_type *q_ptr = &forge;
			object_prep(q_ptr, 605); //石塊
	...
	if (feature_action_flags[action] & FAF_CRASH_GLASS)
	{
		...
		///mod160118 咲夜時間停止中はガラスが砕けても破片が飛ばない
		if (have_flag(old_f_ptr->flags, FF_GLASS) && character_dungeon && !SAKUYA_WORLD)
		{
...
//「旧神の印」の判定
bool is_elder_sign(cave_type *c_ptr)
{
	if ((c_ptr->info & CAVE_OBJECT) && have_flag(f_info[c_ptr->mimic].flags, FF_ELDER_SIGN))
		return TRUE;
	else
		return FALSE;
}
...
///mod150719 布都 皿を置いた地形
bool is_plate_grid(cave_type *c_ptr)
{
	if ((c_ptr->info & CAVE_OBJECT) && have_flag(f_info[c_ptr->mimic].flags, FF_PLATE))
		return TRUE;
	else
		return FALSE;
}
...
void scatter(int *yp, int *xp, int y, int x, int d, int m)
{
	...
	if (!in_bounds(y, x))
	{
		msg_print("ERROR:scatter()に不正なx,yが渡された");
		return;
	}
