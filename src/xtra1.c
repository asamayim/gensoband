...
#define BAR_ROB_MANA			92 //v2.1.1 妖夢専用の「修羅の血」だったが異変領域魔法「スカベンジング」と共用して「魔力奪取」と表示する
...
#define BAR_AMPLIFY_STONE		109 //v2.1.1 異変石増幅
...
#define BAR_NO_MOVE				114 //v2.0.11 移動禁止
#define BAR_TRANSPORTATION_TRAP	115 //v2.0.11 移送の罠
#define BAR_VOID				116 //v2.0.17 残無虚無操作
#define BAR_BEES				117 //v2.0.19 養蜂家の蜂
#define BAR_HIDE				118 //v2.1.0  潜伏
...
static void calc_spells(void)
{
	...
	//v2.1.0 瑞霊は2領域習得しているが実質一領域なので減らす
...
static void calc_torch(void)
{
	...
	//v2.0.13 お燐は他に光源がないとき自前光源+1(尻尾の先が光る)
	if (p_ptr->pclass == CLASS_ORIN && p_ptr->cur_lite < 1)
	{
		p_ptr->cur_lite++;
		if (p_ptr->old_lite != p_ptr->cur_lite)
			msg_print("あなたの尻尾の先が仄かに燃えて辺りを照らした。");
...
*/
	...
	case CLASS_BYAKUREN:
		...
		if(p_ptr->tim_general[0]) //白蓮強化時
		{
			...
			if(p_ptr->lev > 19) p_ptr->resist_pois = TRUE;
			if(p_ptr->lev > 24) p_ptr->sustain_str = TRUE;//v2.0.20 能力維持がFALSEになっていたので修正
			if(p_ptr->lev > 24) p_ptr->sustain_con = TRUE;
			if(p_ptr->lev > 24) p_ptr->sustain_dex = TRUE;
			if(p_ptr->lev > 29) p_ptr->resist_lite = TRUE;
			if(p_ptr->lev > 34) p_ptr->mighty_throw = TRUE;
