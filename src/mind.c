...
/*:::mind_power型はmind_type型の配列　mind_typeはレベル、コスト、失敗率、名前の順の構造体*/
/*:::ここには超能力者、錬気術師、狂戦士、忍者、鏡使いのそれぞれの技の基本情報が格納されている*/
...
/*:::特技の「効果」欄のコメントを得る。超能力者、錬気術師、鏡使い、忍者が対象*/
///class 特技コメント　内容はdo_cmd_mind()のと一致すること
 ...
 * sorry.
 */
/*:::特技の一覧を表示しその中から選択する　超能力者、錬気術師、狂戦士、忍者、鏡使い*/
/*:::sn:選択された番号*/
/*:::only_browse:bコマンドで説明文を読むときTRUE*/
///class
...
put_str(format("Lv   %s   Fail Info", ((use_mind == MIND_BERSERKER) || (use_mind == MIND_NINJUTSU)) ? "HP" : "MP"), y, x + 35);
				...
				for (i = 0; i < MAX_MIND_POWERS; i++)
				{
					...
					if (chance)
					{
						...
						/*:::player_magic.spell_statに記されたパラメータによる失敗率減少*/
 ...
 * is 'ForceTrainer'.
 */
/*:::錬気術師の技　magic_num1[0]に気の溜まった量を格納している*/
...
/*:::攻撃後、ターゲットモンスターの居場所が＠の侵入できる地形でトラップがなく、
 *:::かつターゲットモンスターの一マス後ろも＠の侵入できる地形でトラップもモンスターもない場合、
 *:::＠の移動処理を行う*/
		py_attack(y, x, 0);
...
msg_print("その方向にはモンスターはいません。");
...
#if 0
	{
		...
		for (i = 0; i < 8; i++)
		{
			...
			/*:::↑この記述は？本来レベルが上がるほどmultが上がるはずだった？*/
...
msg_print("混乱していて集中できない！");
	...
	switch(p_ptr->pclass)
	{
		case CLASS_MINDCRAFTER:
		case CLASS_SUMIREKO:
			use_mind = MIND_MINDCRAFTER;p = "精神";break;
...
void do_cmd_mind_browse(void)
{
	...
	while(1)
	{
		...
		/*:::なぜ鏡使いと忍者だけこのメッセージを出す？←特技の種類が多く説明を表示したら一覧の下のほうが隠れるせいらしい*/
		switch (use_mind)
		{
		case MIND_MIRROR_MASTER:
		case MIND_NINJUTSU:
