 ...
 */
/*:::rawファイル(fdにオープン済）を*_info[]に展開する*/
 ...
 */
/*:::指定されたinfoファイルのrawファイルを読み込み??_info[ ]に展開する
 *:::rawファイルがないか現在バージョンより古ければ?_info.txtを読み込み解析してrawファイル生成*/
 ...
 */
///item store BM以外の店に並ぶ基本アイテム　ここのエントリがランダムに呼ばれるのでランダムの範囲を工夫すれば町ごとに品揃えを変えられる。
...
{
	{
		...
		{ TV_SOFTDRINK, SV_DRINK_WATER },//v1.1.78 水やジュースを増やした
 ...
 */
/*:::町を初期化　店は別　マップが切り替わって街に来るたびに呼ばれる*/
///system race class 店のオーナーとかの削除予定
...
static errr init_other(void)
{
	...
	///mod140608 追加インベントリ初期化
	C_MAKE(inven_add, INVEN_ADD_MAX, object_type);
	///mod160206 追加インベントリ初期化2
...
static errr init_alloc(void)
{
	...
	///system 人里にモンスターをでなくするために一工夫いるのかもしれない
#ifdef SORT_R_INFO
 ...
 *:::ファイルの検証　news.txt表示　ハイスコアファイル作成内部変数初期化　設定ファイルロード
 *
 ...
 *:::lib/apexディレクトリが存在しないとハイスコアファイルが作れず落ちる？
 *
 ...
 *:::データの初期化はlib/dataやlib/editのファイルを参照する
 *
 * Note that the "template" files are initialized first, since they
 * often contain errors.  This means that macros and message recall
 * and things like that are not available until after they are done.
 *:::テンプレートファイルが最初に初期化される？
 *
