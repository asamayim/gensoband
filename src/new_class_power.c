...
class_power_type class_power_nina[] =
{
	...
	{ 5,15,20,FALSE,TRUE,A_INT,0,0,"蜃気楼Ⅰ",
	"周囲の敵を混乱させようと試みる。" },

	{ 10,15,30,FALSE,TRUE,A_INT,0,0,"テトラモノアイ",
	"周囲のモンスターを感知する。レベルが上がるとアイテムや地形も感知する。" },

	{ 15,10,0,FALSE,TRUE,A_INT,0,0,"蜃気楼Ⅱ",
	"自分が最近目にした魔法や特技を再現する。威力は目にしたときと同じで発動に追加のコストは必要ないが追加の成功判定が必要。使用したものの記憶は消える。「特別な行動」は再現できない。レベルの上昇に伴い記憶できる個数が増える。" },

	{ 20,30,35,FALSE,TRUE,A_CON,0,0,"ケムトレイルクラウド",
	"一時的に毒の一時耐性を得る。レベルが上がると火炎・冷気・酸・電撃の一時耐性も得る。" },

	{ 25,50,50,FALSE,TRUE,A_INT,0,0,"蜃気楼Ⅲ",
	"ダンジョンの今いる場所に指定した種類の建物を生成する。レベルが上がると作れる建物の種類が増える。ただし周囲に広いスペースがないといけない。一部の特殊な地形には作ることができない。" },

	{ 30,40,60,FALSE,TRUE,A_CHR,0,0,"レプティリアンインテリジェンス",
	"最近打倒したユニークモンスターの幻影を配下として召喚する。幻影の能力は元のモンスターと同じだが一部の特殊行動は使えず、またプレイヤーがフロアから出ると消える。一度召喚したモンスターは候補リストから消える。パラメータが不定などの特殊なモンスターを呼び出すことはできない。" },

	{ 35,30,65,FALSE,TRUE,A_CON,0,0,"アーティフィシャルディザスター",
	"周辺を*破壊*する。" },

	{ 39,80,70,FALSE,TRUE,A_INT,0,0,"タルタリアンラプソディ",
	"今いるフロアをほぼ一瞬で作り変え、新たなフロアには地下街が出現する。難易度EXTRAの場合は次に行くフロアに地下街が出現する。地下街の存在しない一部のダンジョンではフロアを作り変える効果のみが発動。地上やクエスト中のダンジョンでは使用できない。" },

	{ 43,50,75,FALSE,FALSE,A_CHR,0,0,"ニューワールドオーダー",
	"指定したターゲットの周辺にフロアにいる自分の配下をテレポートさせる。さらに配下がすぐに次の行動を行えるようになる。" },

	{ 47,100,80,FALSE,FALSE,A_CHR,0,0,"アンゴルモアの大王",
	"自分の種族を短時間「魔王」に変化させる。またその間は「蜃気楼Ⅱ」で再現する魔法や特技の威力が二倍になる。" },
...
cptr do_cmd_class_power_aux_nina(int num, bool only_info)
{
	...
	{
		int base = 12;
		if (only_info) return format("範囲:%d", base);
	...
	case 7:
	{
		...
		if (use_itemcard)
			msg_print("カードから輝く霧があふれだし辺りを包んだ...");
		else
			msg_print("輝く霧があふれだし辺りを包んだ...");
...
class_power_type class_power_ariya[] =
{
	...
	{ 5,5,25,FALSE,TRUE,A_CON,0,5,"弾幕の化石",
	"無属性のボールで攻撃する。威力はプレイヤーのACに応じて上昇する。" },

	{ 10,15,30,FALSE,TRUE,A_DEX,0,0,"巌となるさざれ石",
	"指定した位置のモンスターにダメージを与える。モンスターがいないときは「岩」地形を生成する。" },

	{ 15,30,40,FALSE,TRUE,A_CHR,0,0,"永遠の冬",
	"視界内の全てを極寒属性で攻撃し、さらにモンスターを短時間移動禁止にする。" },

	{ 20,50,50,FALSE,TRUE,A_CHR,0,0,"混沌の冬",
	"周囲のモンスターを追い払う。クエスト中のフロアでは使えない。" },

	{ 25,35,55,FALSE,FALSE,A_CON,0,10,"ストーンゴッデス",
	"HPを大幅に回復する。" },

	{ 30,50,60,FALSE,TRUE,A_CON,0,0,"幻想のリリクウィー",
	"自分を中心に万能属性の巨大なボールを発生させて攻撃する。威力はプレイヤーのACに応じて上昇する。" },

	{ 35,80,70,FALSE,TRUE,A_DEX,0,0,"錦の上のイマジナリー",
	"フロア全ての壁を永久壁にする。またはフロア全ての永久壁を普通の壁にする。" },

	{ 40,100,80,FALSE,FALSE,A_CON,0,0,"不生不滅の石の女神",
	"一時的に無敵化する。" },

	{ 45,240,90,FALSE,TRUE,A_CHR,0,0,"恒久の冬",
	"現在のフロアでモンスターのあらゆる特殊行動を禁止する。移動や通常攻撃は可能。フロアを移動すると解除される。" },
...
cptr do_cmd_class_power_aux_ariya(int num, bool only_info)
{
	...
	case 4:
	{
		...
		if(plev == 50) msg_print("あなたは一瞬で体を修復した。");
		hp_player(heal);
	...
	case 6:
	{
		...
		put_str("a) 普通の壁を永久壁にする", 10, 20);
		put_str("b) 永久壁を普通の壁にする", 11, 20);
		put_str("(ESC:cancel)", 12, 20);
		while (TRUE)
		{
			c = inkey();
		...
		if (perma)
			msg_print("不変の力がフロアを覆った。");
		else
			msg_print("不変の力がフロアから消えた。");
	...
	case 8:
	{
		...
		{
			msg_print("すでにこのフロアは停止している。");
			return NULL;
		}

		msg_print("あなたはこのフロアの変化を停止させた！");
...
{
	{ 5,10,20,FALSE,FALSE,A_INT,0,0,"情報取得",
	"周囲の地形・トラップ・アイテム・モンスターを感知する。ただしダンジョン「浅間浄穢山」以外では効果範囲が大幅に狭まる。" },

	{ 10,16,30,FALSE,FALSE,A_WIS,0,0,"ストリーミングドール",
	"モンスター一体に混乱属性のダメージを与える。レベル30以降は短時間の移動禁止も追加する。" },

	{ 15,25,40,FALSE,TRUE,A_INT,0,0,"有害情報除去Ⅰ",
	"呪われた装備を解呪する。" },

	{ 20,20,45,FALSE,TRUE,A_WIS,0,10,"ラブルデータ",
	"破片属性のブレスを放つ。威力は現在のMPと等しくなる。" },

	{ 24,30,50,FALSE,FALSE,A_DEX,0,0,"シカを射るだけの美しい記憶",
	"自宅に置かれた矢を手元に呼び出す。もしくは手元の矢を自宅に置く。" },

	{ 28,20,50,FALSE,TRUE,A_INT,0,0,"有害情報除去Ⅱ",
	"隣接したモンスター一体からMPを吸収し加速と無敵化を消去する。またモンスター「虚構人間」「敵意の結晶」を一撃で倒す。" },

	{ 32,40,60,FALSE,TRUE,A_WIS,0,0,"ディスコミュニケーション",
	"視界内の全てのモンスターに敵味方の区別をつかなくさせる。ユニークモンスターと通所の精神を持たないモンスターには効果が薄い。" },

	{ 36,50,70,FALSE,TRUE,A_DEX,0,0,"ディアジェノサイダー",
	"周囲のランダムなモンスターに連続して矢を投擲する。このときの矢は動物に大ダメージを与える。投擲の指輪の効果は半減する。" },

	{ 40,240,95,FALSE,TRUE,A_INT,0,0,"有害情報除去Ⅲ",
	"ザックの中の装備品一つを指定し、反感や太古の怨念などの強力な呪いを除去する。ただし「永遠の呪い」がついた装備品には無効。" },

	{ 44,64,75,FALSE,TRUE,A_CHR,0,0,"スネークランダー",
	"閃光属性の強力なビームを放つ。" },

	{ 48,150,80,FALSE,TRUE,A_CHR,0,0,"ゾルタクスコミュニケーション",
	"視界内全てを因果混乱属性で攻撃する。" },
...
cptr do_cmd_class_power_aux_yuiman(int num, bool only_info)
{
	...
	case 2:
	{
		...
		if (success) msg_print("装備品の呪縛を分解した。");
	...
	case 5://有害情報除去2
	{
		...
		if (cave[y][x].m_idx && (m_ptr->ml))
		{
			...
			if(player_gain_mana(gain_mana)) msg_print("エネルギーを吸い取って魔力に変換した。");
	...
	case 7:
	{
		...
		if (only_info) return  format("回数:最大%d", arrow_num);
	...
	case 8:
	{
		...
		{
			msg_format("蛇は%sを恐れて触れようとしない。", o_name);
			break;
		}

		msg_format("蛇が%sを丸呑みにして吐き出した！", o_name);
...
class_power_type class_power_nareko[] =
{
	...
	{5,0,20,FALSE,TRUE,A_INT,0,0,"謎掛け",
		"人間型生物一体を混乱させようと試み、さらに自分のMPを回復する。友好的なモンスターに使うと怒らせる。通常の精神を持たないモンスターには効果がない。"},

	{10,8,20,FALSE,TRUE,A_WIS,0,0,"道祖神の勘",
		"周囲の地形を感知する。レベルが上がるとトラップとアイテムとモンスターも感知するようになる。"},

	{20,18,25,FALSE,TRUE,A_INT,0,4,"レーザー",
		"閃光属性のビームを放つ。"},

	{25,30,40,FALSE,TRUE,A_INT,0,0,"ロジカルウォール",
		"指定した方向を中心とする隣接3グリッドに壁を生成する。"},

	{30,30,50,FALSE,TRUE,A_INT,0,0,"ピラミダルラビリンス",
		"壁を消滅させるビームを放つ。"},

	{35,25,60,FALSE,TRUE,A_INT,0,0,"エンシャントトラップ",
		"自分のいるグリッドに太古の呪いのトラップを仕掛ける。トラップに踏み込んで判定に失敗したモンスターに全能力低下と移動禁止を付与する。"},

	{40,50,75,FALSE,TRUE,A_INT,0,0,"ロジカルグレートウォール",
		"壁を生成するビームを放つ。"},

	{45,120,90,FALSE,TRUE,A_CHR,0,0,"弾幕の金字塔",
		"指定したターゲットを中心に無属性の大ダメージを与え、さらにターゲットの周囲に壁を生成する。"},
...
cptr do_cmd_class_power_aux_nareko(int num, bool only_info)
{
	...
	switch (num)
	{
		case 0://謎掛け
		{
			...
			if (m_idx && (m_ptr->ml))
			{
				...
				if (one_in_(7))
					...
					msg_print("「朝は四本足、昼は二本足...」");
				else
					msg_print("あなたは謎掛けを挑んだ。");

				r_ptr = &r_info[m_ptr->r_idx];

				if (m_ptr->mflag & MFLAG_SPECIAL)
					msg_format("%sは無言で襲いかかってきた。", m_name);
					...
					msg_format("%sはあなたの問いかけに答えない。", m_name);
				else if (is_pet(m_ptr))
					msg_format("%sはまたかと言いたげに肩をすくめた。", m_name);
				else
				{
					int gain_mana;

					if (m_idx % 42 == 0)
						msg_format("%sは「42」と答えた。なぜか正解のような気がするが不正解だ。", m_name);
					else if (m_idx % 3 == 0)
						msg_format("%sは「人間」と答えた。残念ながら不正解だ。", m_name);
					else if (m_idx % 2 == 0)
						msg_format("%sは「妖怪」と答えた。当然ながら不正解だ。", m_name);
					else
						msg_format("%sはよく分からない生物の名を答えた。よく分からないが不正解だ。", m_name);
					...
					if (player_gain_mana(gain_mana)) msg_print("あなたは精神的に満たされた！");
		...
		case 1: //道祖神の勘
		{
			...
			msg_print("あなたの脳裏に周囲の状況が流れ込んできた。");
			map_area(rad);
...

		{
			...
			if (cave[y][x].m_idx)
			{
				...
				m_ptr = &m_list[cave[y][x].m_idx];
				monster_desc(m_name, m_ptr, 0);
				msg_format("%sの上に巨石が降り注いだ！", m_name);
			}
			else
			{
				msg_print("巨石の山が積み上がった！");
			}
...
class_power_type class_power_aunn[] =
{
	...
	{30,25,40,FALSE,FALSE,A_CON,0,0,"石像化",
		"モンスターから認識されなくなる。視界内に賢いモンスターがいる状態では使用できない。ダメージを受けたり待機、休憩、飲食など以外の行動をすると効果が切れる。"},
	...
	case 3:
		{
			...
			if(flag_ng)
			{
				msg_print("こちらを注意深く見ている敵がいる。いま石像化しても無駄だろう。");
				return NULL;
			}
...
class_power_type class_power_flan[] =
{
	...
	{27,32,60,FALSE,TRUE,A_INT,0,8,"カタディオプトリック",
		"特殊な軌道のビームを5発放つ。"},
	{30,64,70,FALSE,TRUE,A_DEX,0,0,"フォーオブアカインド",
		"分身を三体出現させる。ただし分身は命令に関わらずあなたを範囲攻撃に巻き込む。"},
	{33,30,60,FALSE,TRUE,A_DEX,0,0,"剣生成",
		"一時的に「魔法の剣」を生成して装備する。どちらかの腕が空いているときにしか使えない。この武器を投擲以外で装備から外すことはできない。投擲すると必ず壊れる。" },
...
class_power_type class_power_mystia[] =
{
	...
	{3,3,20,FALSE,TRUE,A_CHR,0,0,"梟の夜鳴声",
		"あなたが今いる部屋を暗くする。"},
	{5,4,30,FALSE,TRUE,A_CHR,0,0,"夜雀の歌",
...
//180
	...
	"狛犬の台座","それを使うと石像化しモンスターから認識されなくなる。視界内に賢いモンスターがいる状態では使用できない。ダメージを受けたり待機、休憩、飲食など以外の行動をすると効果が切れる。"},
