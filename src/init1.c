 ...
 */
 /*:::以下しばらくモンスターのフラグ値
  *:::ここにある文字列に一致するものがr_infoにあれば、そのIDのモンスターのflagsの
  *:::該当位置のフラグがONになる。ここの文字列はRF?_*のフラグ定数値に一致する添え字の場所にあること*/
...
static cptr r_info_flags7[] =
{
	...
	"HAS_DARK_2",
	"SELF_DARK_2",
	"ALWAYS_RUN", //v2.1.3 FULL_MOONから変更した
	"FROM_HELL",
	"VARIABLE",
	"DOPPELGANGER",
	"ONLY_RIDING",
...
static cptr r_info_flagsr[] =
{
	...
	"RES_NETH",
	"RES_WATE",
	"RES_PLAS",//今は使われていないはず
	"RES_SHAR",
	"RES_SOUN",
	"RES_CHAO",
	"RES_HOLY",
	"RES_DISE",
	"RES_WALL",//今は使われていないはず
	"RES_INER",
	"RES_TIME",
	"RES_GRAV",//今は使われていないはず
	"RES_ALL",
	"RES_TELE",
	"XXX",
	"XXX",
 ...
 */
/*:::parse_k_info()から呼ばれる*/
/*:::k/e/a_infoの"STR"などの文字列をアイテムフラグ値TR_STRなどに変換するための配列*/
/*:::この配列の添え字と文字列とTR_???の数値は一致していること*/
///mod131228 item アイテムフラグ入れ替え
 ...
 * the text.
 */
/*:::offsetの位置に対しbufの文字列をコピーし、offsetをその分進め、head->text_sizeを文字数分加算*/
 ...
 * the name.
 */
/*:::offsetの位置に対しbufの文字列をコピーし、header.name_sizeを文字数分加算する*/
 ...
 */
/*:::fpに開いているテキストファイルを指定した関数で一行ずつ処理する*/
 ...
 */
/*:::文字列whatを指定されたnames[]から探し、
 *:::見つかった場所のnames[]の添字分シフトしたflagsのビットを1にする*/
 ...
 */
/*:::よく解らないがf_info.txtのタグからIDに変換してるらしい*/
 ...
 */
/*:::入力した文字列に合致するアイテムのフラグを得る。k_info.txtの解析時にのみ呼ばれる*/
 ...
 */
/*:::発動フラグを得る。k_info/a_info/e_info構文処理から呼ばれる*/
 ...
 */
/*:::アーティファクトのフラグを得る　a_info.txt解釈からのみ呼ばれる*/
/*:::動作はgrab_one_kind_flag()と全く同じ*/
 ...
 */
/*:::モンスターフラグflags1-3,7-9,flagsrを得る*/
 ...
 */
/*:::モンスターフラグflags4-6を得る*/
///system モンスタースペルを増やすならここを触らないといけない
...
errr parse_r_info(char *buf, header *head)
{
	...
	/* Current entry */
	/*:::一度関数が終わってもr_ptrの位置は保持されている*/
	static monster_race *r_ptr = NULL;


	/* Process 'N' for "New/Number/Name" */
	/*:::IDとモンスター名を抽出し、r_ptrをIDの位置に合わせ、モンスター名を記録する*/
	if (buf[0] == 'N')
	{
		/* Find the colon before the name */
		s = my_strchr(buf+2, ':');
	...
	/* Process 'G' for "Graphics" (one line only) */
	/*:::文字と色をd_charとd_attrに格納　色は文字から数字に変換される*/
	else if (buf[0] == 'G')
	{
		char sym;
		byte tmp;
	...
	/* Process 'I' for "Info" (one line only) */
	/*:::加速（表記+110) (speed)、ダイス数(hdice)、ダイス面(hside)、感知範囲(aaf)、AC(ac)、警戒度(sleep)をそれぞれ格納*/
	else if (buf[0] == 'I')
	{
	...
	/* Process 'W' for "More Info" (one line only) */
	/*レベル、レアリティ、経験値、進化までの経験値、進化後IDを格納*/
	else if (buf[0] == 'W')
	{
		int lev, rar, pad;
		long exp;
 ...
 */
/*:::letter[]に格納されたデータがあとでどう参照されるのかわからない。*/
/*:::・・と思ったが、<letter>の文字がintに変換されてletter[]の添え字になるらしいと気が付いた。*/
static errr parse_line_feature(char *buf)
{
	...
	/* Tokenize the line */
	/*:::bufの文字列を:や/で分割してzzに最大9個格納し分割された数を戻り値としてnumに格納*/
	if ((num = tokenize(buf+2, 9, zz, 0)) > 1)
	{
		...
		/*:::ここ解らない。なぜswitch文がここに使われているのか。これじゃトークンの一番最後しか解釈してくれないと思うが。*/
		/*:::と思ってたがswitch文でbreakがなかったらcaseに関係なく下の処理へそのまま進むんだった。すっかり忘れてた。*/
		switch (num)
		{
			/* Special */
			case 9:
			...
			case 7:
				...
				{
					//Hack - Extraモード40階の宝物庫　出現済みだったら☆生成置いとく
					//v1.1.43 ちょっと前に入れた刀三振追加
					if(EXTRA_MODE)
					{
			...
			case 4:
				...
				/*:::c???の場合インデックス-???(ユニークのクローン)が配置される*/
				else if (zz[3][0] == 'c')
				{
					if (!zz[3][1]) return PARSE_ERROR_GENERIC;
					letter[index].monster = - atoi(zz[3] + 1);
				}
				///mod140405 hack モンスターインデックスの頭にwを付加したら最初から起きているようにする
				else if (zz[3][0] == 'w')
				{
					if (!zz[3][1]) return PARSE_ERROR_GENERIC;
...
static errr parse_line_building(char *buf)
{
	...
	/*:::行の途中に漢字が含まれる可能性がある場合この関数を使ってtokenizeする？・・そうでもなかった*/
	/*:::文字列のポインタを指定文字まで進める（漢字対応）*/
	s = my_strchr(s, ':');
 ...
 */
/*::: buf:info.txtの処理する一行 x/y min/max : 生成されるマップの領域？ x,y: 初期ではxminとyminの値が格納されている*/
...
/*:::///patch131222本家rev3510の修正を適用？*/
	...
	if (buf[0] == '%')
	{
		...
		/*:::"%:ファイル名"の構文の解釈 指定されたファイルをさらに読みに行って同じ処理をする*/
		return (process_dungeon_file(buf + 2, ymin, xmin, ymax, xmax));
	}

	/* Process "F:<letter>:<terrain>:<cave_info>:<monster>:<object>:<ego>:<artifact>:<trap>:<special>" -- info for dungeon grid */
	/*:::フロアのグリッドごとの地形情報を宣言するための文？ここでletter[255]の中に<letter>の文字(int)を添え字としてその地形の情報が格納される。*/
	if (buf[0] == 'F')
	{
	...
	else if (buf[0] == 'D')
	{
		...
		for (*x = xmin, i = 0; ((*x < xmax) && (i < len)); (*x)++, s++, i++)
		{
			...
			/*:::letterのinfo.txtで記述されてない文字についてはどこで記述されている？？？*/
			/*:::↑一部の壁などはq_pref.txtに記述されててinit_dungeon_quests()のとき読み込まれている*/
			/*:::あとは主にf_info[]から*/
			...
			/* Only the features */
			/*:::なぜこんなところでcontinueするのかわからない。forの開始からここまでinit_flagsが変わる様子もないからbreakすればよさそうなもんだが*/
			...
			{
				/*::: <*10>とかの処理 monster_levelはグローバル変数*/
			...
			{
				int old_cur_num, old_max_num;/*:::クローン関係の処理用?*/
				bool clone = FALSE;
				...
				/* Place it */
				/*:::place_monster()からもここに来る*/
				///mod140405 hack- クエストinfo.txtのモンスターインデックス番号にwを付けるとletter[idx].randomにフラグが立ってモンスターが起きて配置されるようにした
				...
				if (clone)
				{
					...
					/*:::クローンが死んだとき本物が出てこなくならないようにしてるらしい*/
			...
			else if (object_index)
			{
				...
				if (o_ptr->tval == TV_GOLD)
				{
					...
					/*:::この中でもう一度object_prepが呼ばれているのがよくわからん*/
					/*:::別ルーチンから金を生成させたときのための処理で今回は冗長？*/
					make_gold(o_ptr);
					coin_type = 0;
				}
				...
				/*:::アイテムを指定マスに配置。place_object()の後半でも似た事をしている*/
				drop_here(o_ptr, *y, *x);
			}
			...
			if (artifact_index)
			{
				/*:::Mega Hack - 宝物庫で★がない（全て入手済み、★なしオプション）のときquest[].k_idxにk_info[]添字のidxが入りここで☆生成する。*/
				if(quest[p_ptr->inside_quest].type ==  QUEST_TYPE_FIND_RANDOM_ARTIFACT)
				{
					object_type forge;
				...
				{
					/*:::すでに該当アーティファクトが出現済みの場合は獲得の巻物を生成*/
					//↑☆生成にした
					//↑普通のクエスト報酬でガラドリエルが☆生成になってしまったので直す。街マップもここで生成されるのを忘れていた。
					int k_idx;
					object_type forge;
					object_type *q_ptr = &forge;

					//v1.1.13 永遠亭のクエスト報酬(五つの難題)の報酬を持っている場合も☆生成にする
	...
	else if (buf[0] == 'Q')
	{
		...
		else if (zz[1][0] == 'R')
		{
			if (init_flags & INIT_ASSIGN)
			{
				...
				/*:::インデックス数列の中からランダムにreward_idxに格納してる。n個目のアイテムを1/(n)の確率で置き換えることで全て均等確率で割り振られるようだ*/
				...
				else
				{
					...
					///mod151028 特殊処理　宝物庫★なしのときはk_idxにk_infoのindexを入れ、ランダムアーティファクトにする
	...
	/*:::misc.txtを解釈する 様々な要素のMAX値をグローバル変数に格納*/
	else if (buf[0] == 'M')
	{
		if (tokenize(buf+2, 2, zz, 0) == 2)
		{
 ...
 */
/*:::info.txtの"?:[～]の構文を解釈。おそらく「条件を満たしたときのみ次の行を実行する」ための条件判定処理。*/
/*:::最終的にvが0で終わるとき、info.txtの次の行がスキップされる*/
static cptr process_dungeon_file_expr(char **sp, char *fp)
{
	...
	/* Default */
	/*:::*vのアドレス部分を"?o?o?"って文字列のアドレスにして*vに?o?o?を代入*/
	v = "?o?o?";
	...
	if (*s == b1)
	{
		...
		/* Function: NOT */
		/*::: NOT? もし複数の条件式がこれに続いたら一つでも1なら結果は0になる？*/
		else if (streq(t, "NOT"))
		{
			v = "1";
			while (*s && (f != b2))
		...
		/* Function: LEQ */
		/*::: 変数二つのうち後者のほうが大きければ1? 文字列なら最初の一文字のみ比較？*/
		else if (streq(t, "LEQ"))
		{
			v = "1";
			if (*s && (f != b2))
		...
		/* Function: GEQ */
		/*::: 変数二つのうち前者のほうが大きければ1? 文字列なら最初の一文字のみ比較？*/
		else if (streq(t, "GEQ"))
		{
			v = "1";
			if (*s && (f != b2))
	...
	else
	{
		...
		if (*b == '$')
		{
			...
			else if (prefix(b+1, "QUEST"))
			{
				...
				/*:::atoiは指定したアドレスから数字が続いている限り何文字でもintに変換するらしい*/
				sprintf(tmp, "%d", quest[atoi(b+6)].status);
				v = tmp;
			}

			///mod140401 草の根妖怪ネットワーク加入時に1
			...
			///mod140113 再挑戦時かどうかの判定追加 ?:[EQU $RETRY(クエスト番号) 1]　のように使う
...
/*:::～_info.txtを解釈し、クエストダンジョンを生成したりマップを生成したり町の設備の情報をグローバル変数に格納する*/
/*:::x/y min/max は生成されるダンジョン、マップの座標範囲？*/
...
errr process_dungeon_file(cptr name, int ymin, int xmin, int ymax, int xmax)
{
	...
	/* Process the file */
	/*:::ここで読まれる行はマップだけでなくダンジョンやクエストのパラメータ類も含む*/
	...
	/*:::///patch131222本家rev3510の修正を適用？*/
		...
		if ((buf[0] == '?') && (buf[1] == ':'))
		{
			...
			/* Start */
			/*:::sにbufの三文字目のアドレスを入れている。つまり?:の後の場所*/
			s = buf + 2;

			/* Parse the expr */
			/*:::info_txtの"?:[～]の構文を解釈。「条件を満たしたときのみ次以降の行を実行する」処理。*/
			...
			/* Continue */
			/*:::次の行を読みに行っている。bypass以外は残らない*/
			continue;
		}

		/* Apply conditionals */
		/*bypassがTrue(?:[～]が前の行にあったが条件を満たさない)のときその次以降の行はパスされる。これは?:行でbypassがFALSEになるまで続くらしい*/
		if (bypass) continue;

		/* Process the line */
		/*:::各行を解釈し実行。地形やモンスターやアイテムを配置する*/
