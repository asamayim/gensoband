...
#define FLG_SOUVENIR		19
#define FLG_RIBBON			20
#define FLG_ALCOHOL			21
...
#define FLG_BOOTS            49
//v1.1.32
#define FLG_SWEETS	     50
#define FLG_SOFTDRINKS   51
//v1.1.50
#define FLG_ITEMCARDS	52

//最大63?

#define FLG_NOUN_BEGIN      FLG_ITEMS
#define FLG_NOUN_END        FLG_ITEMCARDS
...
static char KEY_SOUVENIR[] = "珍品";
static char KEY_RIBBON[] = "リボン";
static char KEY_ALCOHOL[] = "酒";
...
static char KEY_SWEETS[] = "菓子";
static char KEY_SOFTDRINKS[] = "飲料";
static char KEY_ITEMCARDS[] = "カード"; //「アイテムカード」にすると上の「アイテム」の処理が先に行われてうまく動かなかった
 ...
 */
/*:::自動拾いファイルの各行からautopick_list[]用のエントリへの変換？*/
static bool autopick_new_entry(autopick_type *entry, cptr str, bool allow_default)
{
	...
	while (old_ptr != ptr)
	{
		...
		//if (MATCH_KEY(KEY_SOUVENIR)) ADD_FLG(FLG_SOUVENIR);
	//	if (MATCH_KEY(KEY_RIBBON)) ADD_FLG(FLG_RIBBON);
	...
	else if (MATCH_KEY2(KEY_ALCOHOL)) ADD_FLG_NOUN(FLG_ALCOHOL);
	else if (MATCH_KEY2(KEY_SOUVENIR)) ADD_FLG_NOUN(FLG_SOUVENIR);
	else if (MATCH_KEY2(KEY_RIBBON)) ADD_FLG_NOUN(FLG_RIBBON);
	else if (MATCH_KEY2(KEY_MATERIALS)) ADD_FLG_NOUN(FLG_MATERIALS);
	...
	else if (MATCH_KEY2(KEY_SWEETS)) ADD_FLG_NOUN(FLG_SWEETS);
	else if (MATCH_KEY2(KEY_SOFTDRINKS)) ADD_FLG_NOUN(FLG_SOFTDRINKS);
	else if (MATCH_KEY2(KEY_ITEMCARDS)) ADD_FLG_NOUN(FLG_ITEMCARDS);
 ...
 */
/*:::アイテムからautopick用エントリを作成する?　アイテムの種類、質、鑑定状況に応じautopick_type型に数値を格納*/
///item class アイテム分類関係編集要
...
///del131221 賞金首
	...
	if (o_ptr->tval <= TV_BOOK_END &&
		!(o_ptr->tval == TV_BOOK_MEDICINE && (CHECK_MAKEDRUG_CLASS)) &&
	...
	if (p_ptr->realm1 == o_ptr->tval && !(REALM_SPELLMASTER))
	...
	if (p_ptr->realm2 == o_ptr->tval && !(REALM_SPELLMASTER))
	//    p_ptr->pclass != CLASS_SORCERER &&
	 //   p_ptr->pclass != CLASS_RED_MAGE)
	{
		ADD_FLG(FLG_REALM2);
	...
	if (o_ptr->tval <= TV_BOOK_END && 0 == o_ptr->sval)
		ADD_FLG(FLG_FIRST);
	if (o_ptr->tval <= TV_BOOK_END && 1 == o_ptr->sval)
		ADD_FLG(FLG_SECOND);
	if (o_ptr->tval <= TV_BOOK_END && 2 == o_ptr->sval)
		ADD_FLG(FLG_THIRD);
	if (o_ptr->tval <= TV_BOOK_END && 3 == o_ptr->sval)
		ADD_FLG(FLG_FOURTH);
	...
	///mod140426
		...
		ADD_FLG(FLG_MATERIALS);
	else if (o_ptr->tval == TV_RIBBON)
		ADD_FLG(FLG_RIBBON);
	else if (o_ptr->tval == TV_SOUVENIR)
		ADD_FLG(FLG_SOUVENIR);
	else if (o_ptr->tval <= TV_BOOK_END)
		...
		ADD_FLG(FLG_AMULETS);
	else if (o_ptr->tval == TV_DRAG_ARMOR || o_ptr->tval == TV_ARMOR ||
		...
		ADD_FLG(FLG_SWEETS);
	else if (o_ptr->tval == TV_SOFTDRINK)
		ADD_FLG(FLG_SOFTDRINKS);
	else if (o_ptr->tval == TV_ITEMCARD)
		ADD_FLG(FLG_ITEMCARDS);
 ...
 */
/*:::autopick_type型の変数をautopick_list[]に追加*/
 ...
 */
/*:::自動拾い設定ファイルの読み込んだ一行をautopick_list[]に読み込む*/
 ...
 */
/*:::autopick_type型から自動拾い設定ファイル用記述への変換*/
cptr autopick_line_from_entry(autopick_type *entry)
{
	...
	*buf = '\0';
	...
	ptr = buf;
	/*:::注：ADD_KEY()マクロはptrを対象にするよう固定されている*/
	...
	else if (IS_FLG(FLG_SOUVENIR)) ADD_KEY2(KEY_SOUVENIR);
	else if (IS_FLG(FLG_ALCOHOL)) ADD_KEY2(KEY_ALCOHOL);
	else if (IS_FLG(FLG_RIBBON)) ADD_KEY2(KEY_RIBBON);
	...
	else if (IS_FLG(FLG_MATERIALS)) ADD_KEY2(KEY_MATERIALS);
	...
	else if (IS_FLG(FLG_SWEETS)) ADD_KEY2(KEY_SWEETS);
	else if (IS_FLG(FLG_SOFTDRINKS)) ADD_KEY2(KEY_SOFTDRINKS);
	else if (IS_FLG(FLG_ITEMCARDS)) ADD_KEY2(KEY_ITEMCARDS);
 ...
 */
/*:::autopick_typeから自動拾いファイル用記述への変換をし、変換したエントリをautopick_list[]から削除*/
 ...
 */
/*:::アイテム一つと自動拾いルール一行の一致判定*/
///item class realm TVAL 自動拾いファイル　重要
static bool is_autopick_aux(object_type *o_ptr, autopick_type *entry, cptr o_name)
{
	...
	if (IS_FLG(FLG_BOOSTED))
	{
		...
		if (!object_is_melee_weapon(o_ptr) && o_ptr->tval != TV_GUN)
			return FALSE;
		...
		//未鑑定の銃はダイスブーストされてても対象にならない
		//if (!object_is_known(o_ptr) && o_ptr->tval == TV_GUN)
		//	return FALSE;
	...
	*/
	...
	///realm class 領域判定
	if (IS_FLG(FLG_REALM1) && !(CHECK_MAKEDRUG_CLASS && o_ptr->tval == TV_BOOK_MEDICINE)  &&
	    (p_ptr->realm1 != o_ptr->tval || REALM_SPELLMASTER ))
		return FALSE;

	/*** Second realm spellbooks ***/
		///realm class 領域判定
	if (IS_FLG(FLG_REALM2) && 
	    (p_ptr->realm2 != o_ptr->tval || REALM_SPELLMASTER))
		return FALSE;

	///realm item TVAL 魔法書判定　ここから下　
	/*** First rank spellbooks ***/
	if (IS_FLG(FLG_FIRST) &&
	    (o_ptr->tval > TV_BOOK_END || 0 != o_ptr->sval))
		return FALSE;

	/*** Second rank spellbooks ***/
	if (IS_FLG(FLG_SECOND) &&
	    (o_ptr->tval > TV_BOOK_END || 1 != o_ptr->sval))
		return FALSE;

	/*** Third rank spellbooks ***/
	if (IS_FLG(FLG_THIRD) && 
	    (o_ptr->tval > TV_BOOK_END || 2 != o_ptr->sval))
		return FALSE;

	/*** Fourth rank spellbooks ***/
	if (IS_FLG(FLG_FOURTH) &&
	    (o_ptr->tval > TV_BOOK_END || 3 != o_ptr->sval))
		return FALSE;
...
#endif
	///mod140426　素材など追加
	else if (IS_FLG(FLG_ALCOHOL))
	{
		if (o_ptr->tval != TV_ALCOHOL)
			return FALSE;
	}
	else if (IS_FLG(FLG_MATERIALS))
	{
		if (o_ptr->tval != TV_MATERIAL)
			return FALSE;
	}
	else if (IS_FLG(FLG_SOUVENIR))
	{
		if (o_ptr->tval != TV_SOUVENIR)
			return FALSE;
	}
	else if (IS_FLG(FLG_RIBBON))
	{
		if (o_ptr->tval != TV_RIBBON)
			return FALSE;
	}
	///item TVAL 自動拾い　しばらく下まで
	...
	else if (IS_FLG(FLG_SWEETS))
	{
		if (!(o_ptr->tval == TV_SWEETS))
			return FALSE;
	}
	else if (IS_FLG(FLG_SOFTDRINKS))
	{
		if (!(o_ptr->tval == TV_SOFTDRINK))
			return FALSE;
	}
	else if (IS_FLG(FLG_ITEMCARDS))
	{
 ...
 */
/*:::指定アイテムがautopick_list[]のルールのいずれかに当たるかどうか判定*/
...
static bool is_opt_confirm_destroy(object_type *o_ptr)
{
	...
	///mod160327
	if(o_ptr->tval == TV_ITEMCARD) return FALSE;
	//v1.1.86
	if (o_ptr->tval == TV_ABILITY_CARD) return FALSE;
	//v1.1.56
	if (o_ptr->tval == TV_STRANGE_OBJ) return FALSE;
	if (o_ptr->tval == TV_SPELLCARD) return FALSE;
	...
	if (leave_special)
	{
		...
		if (p_ptr->pclass == CLASS_NINJA)
		{
			if (o_ptr->tval == TV_LITE &&  o_ptr->name2 == EGO_LITE_DARKNESS && object_is_known(o_ptr))
				return FALSE;
			if (o_ptr->tval == TV_STAFF && o_ptr->sval == SV_STAFF_DARKNESS 
				&& object_is_aware(o_ptr))	return FALSE;
			if (o_ptr->tval == TV_SCROLL && o_ptr->sval == SV_SCROLL_DARKNESS 
				&& object_is_aware(o_ptr))	return FALSE;
		}
		else if (cp_ptr->skill_aptitude[SKILL_RIDING] > 3) //騎兵と魔獣使い全般だったのを騎乗B以上にした
		{
			if (o_ptr->tval == TV_WAND && o_ptr->sval == SV_WAND_HEAL_MONSTER && object_is_aware(o_ptr))
				return FALSE;
			else if (o_ptr->tval == TV_WAND && o_ptr->sval == SV_WAND_HASTE_MONSTER && object_is_aware(o_ptr))
				return FALSE;
		}
		else if (p_ptr->pclass == CLASS_MEDICINE) //メディスン　毒の薬、猛毒の薬
		{
			if (o_ptr->tval == TV_POTION &&  o_ptr->sval == SV_POTION_POISON && object_is_known(o_ptr))
				return FALSE;
			if (o_ptr->tval == TV_POTION &&  o_ptr->sval == SV_POTION_POISON2 && object_is_known(o_ptr))
				return FALSE;
		}
		else if (p_ptr->pclass == CLASS_MARISA) //魔理沙　キノコ
		{
		...
		//v1.1.32 薬師の合成薬
		else if(p_ptr->pclass == CLASS_CHEMIST)
		{
			if (o_ptr->tval == TV_COMPOUND)
				return FALSE;
		}
		//v1.1.32 エンジニアの弾薬類
		else if(p_ptr->pclass == CLASS_ENGINEER || p_ptr->pclass == CLASS_NITORI)
		{
		...
		{
			if (o_ptr->tval == TV_MATERIAL && o_ptr->sval == SV_MATERIAL_STONE 
				&& object_is_aware(o_ptr))	return FALSE;
		}
		else if (p_ptr->prace == RACE_VAMPIRE)
		{
			if (o_ptr->tval == TV_STAFF && o_ptr->sval == SV_STAFF_SUMMONING 
				&& object_is_aware(o_ptr))	return FALSE;
			if (o_ptr->tval == TV_SCROLL && o_ptr->sval == SV_SCROLL_SUMMON_MONSTER 
				&& object_is_aware(o_ptr))	return FALSE;
		}
		else if (p_ptr->prace == RACE_KARASU_TENGU)
		{
			if (o_ptr->tval == TV_PHOTO )	return FALSE;
		}
...
/*:::最後に自動破壊されたアイテムが格納される （簡易自動破壊オプション含む）*/
static object_type autopick_last_destroyed_object;


/*:::アイテム自動破壊処理　この時点では自動破壊アイテムにマーク付けられるだけ*/
 ...
 */
/*:::OM_AUTODESTROYのマークがついたアイテムを破壊する*/
static void autopick_delayed_alter_aux(int item)
{
	...
	if (o_ptr->k_idx && (o_ptr->marked & OM_AUTODESTROY))
	{
		...
		///mod141004 鍛冶武器や妖器は自動破壊されない
		if(object_is_smith(o_ptr)) return;

		//大丈夫だとは思うが、小傘の本体を壊さないようにしとく
		if(o_ptr->tval == TV_STICK && o_ptr->sval == SV_WEAPON_KOGASA) return;

		//大丈夫だとは思うが、珍品2とスペカを自動破壊しないようにしておく
		if (o_ptr->tval == TV_STRANGE_OBJ) return;
		if (o_ptr->tval == TV_SPELLCARD) return;
...
void autopick_delayed_alter(void)
{
	...
	///mod140428 @の足元のアイテムを自動破壊したとき足元アイテム一覧を更新
 ...
 * requested.
 */
/*:::所持品か足元の品に自動破壊・自動銘刻み処理　下のpickup_itemsのほうが細かいので詳細はそっち*/
 ...
 */
/*:::足元アイテム全てに自動拾い・破壊を適用　move_player_effect()の下のcarry()から呼ばれる*/
...
static cptr *read_pickpref_text_lines(int *filename_mode_p)
{
	...
	*filename_mode_p = PT_WITH_PNAME;
	...
	/*:::それもなければデフォルトのpicktype.prfをコピーしてくる*/
	if (!lines_list)
	{
...
#define EC_KK_SOUVENIR	       54
#define EC_KK_RIBBON	       55
#define EC_KK_ALCOHOL	       56
...
#define EC_KK_GUNS	       70
//#define EC_KK_CORPSES	       71
#define EC_KK_MATERIALS	       71
...
#define EC_KK_SWEETS	       82
#define EC_KK_SOFTDRINKS	       83
#define EC_KK_ITEMCARDS			84
...
//	{MN_HUMAN, 1, -1, EC_OK_HUMAN},
	...
	{KEY_GUNS, 1, -1, EC_KK_GUNS},
//	{KEY_CORPSES, 1, -1, EC_KK_CORPSES},
	{KEY_MATERIALS, 1, -1, EC_KK_MATERIALS},
	...
	{ KEY_SWEETS, 1, -1, EC_KK_SWEETS },
	{ KEY_SOFTDRINKS, 1, -1, EC_KK_SOFTDRINKS },
	{ KEY_ITEMCARDS, 1, -1, EC_KK_ITEMCARDS },
...
static bool do_editor_command(text_body_type *tb, int com_id)
{
...
#ifdef JP
		(void)show_file(TRUE, "teditor.txt", NULL, 0, 0);
#else
	...
	case EC_INSERT_BLOCK:
	{
		...
		sprintf(expression, "?:[AND [EQU $RACE %s] [EQU $CLASS %s] [EQU $PERSONALITY %s] [GEQ $LEVEL %02d]]", 
#ifdef JP
	...
	case EC_KK_GUNS: toggle_keyword(tb, FLG_GUNS); break;
		///del131221 死体自動拾い
	//case EC_KK_CORPSES: toggle_keyword(tb, FLG_CORPSES); break;
	case EC_KK_MATERIALS: toggle_keyword(tb, FLG_MATERIALS); break;
	...
	case EC_KK_ALCOHOL: toggle_keyword(tb, FLG_ALCOHOL); break;
	case EC_KK_SOUVENIR: toggle_keyword(tb, FLG_SOUVENIR); break;
	case EC_KK_RIBBON: toggle_keyword(tb, FLG_RIBBON); break;
	//v1.1.62 追加
	case EC_KK_SWEETS:toggle_keyword(tb, FLG_SWEETS); break;
	case EC_KK_SOFTDRINKS:toggle_keyword(tb, FLG_SOFTDRINKS); break;
	case EC_KK_ITEMCARDS:toggle_keyword(tb, FLG_ITEMCARDS); break;
...
void do_cmd_edit_autopick(void)
{
	...
	/*:::最後に自動破壊？したアイテムの情報をautopick_list[]に追加*/
