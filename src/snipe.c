...
{
	if(!(CLASS_USE_CONCENT)){ msg_print("ERROR:登録していない職業でreset_concentration()が呼ばれた");return;}
	if(!p_ptr->concent) return;

	if (msg)
	{
		if(p_ptr->pclass == CLASS_KISUME)
			msg_print("あなたは地面まで降下した。");
		...
		else if (p_ptr->pclass == CLASS_RESEARCHER)
		{
			if(p_ptr->magic_num1[0] == CONCENT_KANA)
				...
				msg_print("魔力の集中が元に戻った。");

		}
		else
			msg_print("ERROR:この職業の集中妨害メッセージが設定されていない");
	}
