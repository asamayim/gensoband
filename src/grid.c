...
void place_random_stairs(int y, int x)
{
	...
	/* Bottom */
	///mod140407 Easyでは50階までしか行けない
	//if (dun_level >= d_info[dungeon_type].maxdepth)
	if (dun_level >= d_info[dungeon_type].maxdepth || difficulty == DIFFICULTY_EASY && dungeon_type == DUNGEON_ANGBAND &&  dun_level >= 50)
		down_stairs = FALSE;
...
static bool set_tunnel(int *x, int *y, bool affectwall)
{
	...
	//v1.1.61 PERMANENTフラグ付き地形(店入り口とか)にはトンネルを掘らない.
	//..ようにしたが入り口が消える。トンネル処理が原因ではないらしい。
	//if (cave_have_flag_bold((*y), (*x), FF_PERMANENT)) return TRUE;
