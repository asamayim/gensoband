...
bool potion_smash_effect2(int who, int y, int x, int sval)
{
	...
	case SV_COMPOUND_BUGKILLER: //v2.1.3 蚊除けの薬草酒
...
void cast_musou_hu_in(int reimu_idx)
{
	...
	if (num > 9) num = 9;//v2.0 深層の夢想封印を弱体化
...
//養蜂家が回収可能な蜂蜜を増やす
//p_ptr->magic_num1[0]に蜂蜜の個数*100を記録する
//難易度EXTRAでは1フロア降りる毎、それ以外では6:00と18:00に呼ばれる
void	add_honey(void)
{
	int plev = p_ptr->lev;
	...
	{
		msg_print("ERROR:養蜂家以外でadd_honey()が呼ばれた");
		return;
	}
	...
	if (cheat_peek) msg_format("蜂蜜入手:%d.%02d",add_num/100,add_num%100);
