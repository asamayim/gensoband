 ...
 */
/*:::何か事前に設定しておいた機能を実行するコマンドらしい　詳細不明*/
...
void Term_queue_bigchar(int x, int y, byte a, char c, byte ta, char tc)
{
	...
	else if (' ' <= c) /* isprint(c) */
	{
		...
		//v1.1.55 全角表示にしたらところどころ表示が変になってフリーズしたことがあるので念のためにここにオーバーフロー避けを入れとく
		if (2 * (c - ' ')+1 >= sizeof(ascii_to_zenkaku))
		{
			a2 = TERM_RED;
			c2 = 'x';
 ...
 */
/*:::サブウィンドウに対するカーソル移動　どのサブウィンドウに対しての処理なのかは、
 *:::事前にTermにアドレスを代入しておく事で指定される。*/
 ...
 */
/*:::メイン画面指定位置にカーソルを移動し、そこからn文字をクリア*/
/*::: x:col y:row */
 ...
 */
/*:::この関数が呼ばれたときから指定時間内に押されたキーを取得？詳細未読*/
