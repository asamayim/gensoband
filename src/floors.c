	...
	for (i = 0; i < MAX_SAVED_FLOORS; i++)
	{
		...
		if (fd < 0)
		{
			if (!force)
			{
				...
				msg_print("幻想蛮怒を二重に起動していないか確認してください。");
				msg_print("過去に幻想蛮怒がクラッシュした場合は一時ファイルを");
 ...
 */
/*:::フロア関係のテンポラリファイルをすべて削除する　ゲーム終了時に呼ばれる*/
 ...
 */
/*:::saved_floors[]から指定したIDのフロアのポインタを得る*/
...
s16b get_new_floor_id(void)
{
	...
	{
		int oldest = 0;
		u32b oldest_visit = 0xffffffffL; /*:::-(2^31)？最小値*/
...
//フロア移動するときに連れていけるモンスター数
///v1.1.13 豊姫実装に伴い21→255に拡張
...
static void preserve_pet(void)
{
	...
	if (!p_ptr->wild_mode && !p_ptr->inside_arena && !p_ptr->inside_battle)
	{
		for (i = m_max - 1, num = 1; (i >= 1 && num < MAX_PARTY_MON); i--)
		{
			...
			{
				///v1.1.13 レベル30以降の豊姫は全ての配下を次のフロアに連れていける
			}
			else
			{
 ...
 */
//preserve_pet()で保持した配下リストのモンスターを新たなフロアに配置する
static void place_pet(void)
{
	int i;
	//:::広域モードのときは乗馬モンスターのみが保持されているので一体分だけ処理
	int max_num = p_ptr->wild_mode ? 1 : MAX_PARTY_MON;
	...
	for (i = 0; i < max_num; i++)
	{
		...
		//v1.1.13 レベル30以降の豊姫は大量の配下を連れてこれる
		else if(p_ptr->pclass == CLASS_TOYOHIME && p_ptr->lev > 29)
		{
			int j, d;
...
static void update_unique_artifact(s16b cur_floor_id)
{
	...
	for (i = 1; i < m_max; i++)
	{
		...
		if ((r_ptr->flags1 & RF1_UNIQUE) ||
			...
			( (r_ptr->flags7 & RF7_VARIABLE) && (r_ptr->flags7 & RF7_UNIQUE2)) ///mod140624 霊夢・魔理沙・ランダムユニーク指定
			)
		{
 ...
 */
/*:::階移動、帰還後処理*/
/*:::dungeon()のメインループを抜けて、ゲーム終了も死亡もしてないときここに来る　ほかはアリーナで負けた時とか*/
void leave_floor(void)
{
	...
	/*:::拾ったアーティファクトについて「どこの階にあるか」の情報を破棄している*/
	//この処理必要なのか？0にしなかったら何か不具合があるのかがわからない。
	...
	///mod150925
	/*:::床に置いて行ったアーティファクトに「どこの階にあるか」の情報を再設定することにした。階移動で床の★が消えないように*/
	for (i = 1; i < o_max; i++)
	{
		object_type *o_ptr = &o_list[i];
		if (!o_ptr->k_idx) continue;
 ...
 * will be generated.
 */
/*:::ダンジョンを生成する。すでに知っている階の場合テンポラリファイルからリストアしてくる*/
void change_floor(void)
{
	...
	//v1.1.58 モンスター個別BGMを流している場合リセット
	music_r_idx = 0;
	...
	/*:::地表にいるときにここに来たら新しいダンジョンを生成するらしい。でもなぜFIRST_FLOORがFALSEのときなのか？*/
	/*:::たぶんここの処理は荒野かアリーナだけが対象*/
	if (!(change_floor_mode & CFM_SAVE_FLOORS) &&
	...
	{
		///system 階全体用特殊vaultとか追加するならこの辺りか
		/* No floor_id yet */
		/*:::new_floor_id: floor_id of the destination これから行く階の情報がない（まだ作られていない）場合*/
		if (!new_floor_id)
		{
			/* Get new id */
			/*:::saved_floors[]から空いてる（もしくは一番古い)項目を初期化し、floor_id(saved_floors[]の添え字ではない)を返す*/
			new_floor_id = get_new_floor_id();
		}

		/* Pointer for infomations of new floor */
		/*:::これから行く階のidからsaved_floor_type型のポインタを取得*/
		sf_ptr = get_sf_ptr(new_floor_id);

		/* Try to restore old floor */
		/*:::すでに行ったことのある階(last_visitが真)*/
		if (sf_ptr->last_visit)
		{
			/* Old saved floor is exist */
			/*:::ゲーム中にsave内に作られるフロアデータセーブファイル群からsavefile_idで示されるファイルを読み込みsf_ptrに格納　mode:色々*/
			if (load_floor(sf_ptr, 0))
			{
				loaded = TRUE;

				/* Forbid return stairs */
				/*:::ここ詳細不明 CFM_NO_RETURNはランクエ階など一度出たら戻れないフロアのフラグと思われる*/
				/*:::ランクエ階から出たとき階段を消す処理？*/
		 ...
		 */
		/*:::テレポレベルや落とし戸や階段生成で同じ階層の違うフロアに出ることはない？*/
		if (p_ptr->floor_id)
		{
		...
		if (loaded)
		{
			...
			/* Maintain monsters */
			/*:::一度階を離れると、ペット以外のモンスターはHPが全快し魔法の一時効果が切れる*/
			for (i = 1; i < m_max; i++)
			{
				monster_race *r_ptr;
				monster_type *m_ptr = &m_list[i];
				...
				/*:::別の階に出現したユニークは元の階から消える*/
				if (r_ptr->floor_id != new_floor_id)
				{
					if(cheat_hear) 
						msg_format("ユニーク(ID:%d)が別階層での再登場によりこの階から消えた",m_ptr->r_idx);
					/* Disapper from here */
					delete_monster_idx(i);
			...
			for (i = 1; i < o_max; i++)
			{
				...
				/* Appear at a different floor? */
				//別の階で★が出現したとき、ロードされたフロアの床から★が消える。
				//＠が床に落とした★もfloor_id=0なのでその判定に含まれてしまう。
				//leave_floor()で床上の★全てにそのフロアのidを再設定することでこれを防止した。
		...
		//Extraモードはランダムな場所に出るがクエストだけは初期位置
	...
	if(p_ptr->pclass == CLASS_SEIJA && flag_seija_quest_comp)
	{
		...
		msg_print("あなたは頼み事を聞いて感謝されてしまった！力が抜ける・・");
...
//		msg_print("床上のアイテムが呪文を跳ね返した。");
		msg_print("床上のアイテムが邪魔で階段を作れなかった。");
#else
