 ...
 */
/*:::playrecord-xxx.txtに日記を書く*/
///sys 日記　しばらくは強制的に何もせずreturnにしておこうか
...
//		         && !((q_idx == QUEST_TAISAI) || (q_idx == QUEST_SERPENT))))
		...
		{
			sprintf(note_level_buf, "エクストラクエスト(%d階):", dun_level);
	...
	switch(type)
	{
		...
		case NIKKI_ARENA:
		{
			...
			if (num < 0)
			{
				fprintf(fff, " %2d:%02d %20s 「%s」の夢の住人たちに敗れ去った。\n", hour, min, note_level, nightmare_stage_table[nightmare_rank].desc);
			}
			//勝利
			else
			{
				fprintf(fff, " %2d:%02d %20s 「%s」の夢の住人たちに勝利した。\n", hour, min, note_level, nightmare_stage_table[nightmare_rank].desc);			
			}
...
static void do_cmd_disp_nikki(void)
{
	...
	if(p_ptr->psex == SEX_MALE)
		sprintf(nikki_title, "「%s%s%sの伝説 -%s-」",ap_ptr->title, ap_ptr->no ? "の" : "", player_name, tmp);
	else
		sprintf(nikki_title, "「%s%s%sの伝説 -%s-」",ap_ptr->f_title, ap_ptr->no ? "の" : "", player_name, tmp);
#else
...
void do_cmd_change_name(void)
{
	...
	while (1)
	{
		...
		/* Display the player */
		/*:::突然変異をCコマンドで表示するオプションがONのとき、ここからmode=4で入る*/
		display_player(mode);
		...
		if(cp_ptr->flag_only_unique) 
			Term_putstr(2, 23, -1, TERM_WHITE, "['c'で職業名変更, 'f'でファイルへ書出, 'h'でモード変更, ESCで終了]");
		else
			Term_putstr(2, 23, -1, TERM_WHITE, "['c'で名前変更, 'f'でファイルへ書出, 'h'でモード変更, ESCで終了]");
#else
		...
		if (c == 'c')
		{
			///system ユニークプレイヤーなら職業名を変更するようにしておく
 ...
 */
/*:::メッセージ履歴を表示する　詳細未読だがたぶんこのまま使えるだろう*/
...
static void do_cmd_options_cheat(cptr info)
{
	...
	while (TRUE)
	{
		...
		if(p_ptr->pseikaku == SEIKAKU_COLLECTOR)
		{
			...
			prt("                     <<  注意  >>", 11, 0);
			prt("      詐欺オプションを一度でも設定すると、", 12, 0);
			prt("      次の蒐集家にエントランスを引き継げなくなります。", 13, 0);
...
void do_cmd_options(void)
{
	...
	while (1)
	{
		...
		{
			/* Give some choices */
			/*:::option_fields[][]を参照してオプションメニューを表示　カーソルのある行は色を変える*/
			for (i = 0; i < n; i++)
			{
				byte a = TERM_WHITE;
				if (i == y) a = TERM_L_BLUE;
...
void do_cmd_feeling(void)
{
	...
	else if (p_ptr->town_num && !dun_level)
	{
		...
		{
#ifdef JP
			///sys 街の雰囲気　種族や＠により「友好的/排他的」のメッセージを出してもいいかも
			msg_print("典型的な町のようだ。");
#else
	...
	///system　muta 白オーラ時の階の雰囲気　中身少し変えよう
	/* Display the feeling */
...
///del131211 コンバットmsg
	...
	for (j = 1; j < m_max; j++) 
	{
		...
		//v1.1.48 反獄王に取り憑かれた魔理沙がいるときも通知することにした
		if(r_ptr->flags3 & RF3_WANTED || r_ptr->flags2 & RF2_TROPHY) msg_print("この階にお尋ね者がいるような気がする...");
	}
 ...
 */
/*:::グループ化されるモンスターシンボルの表記　monster_group_char[]と一致すること*/
///mon sys モンスターシンボル・グループ
///mod131231 シンボル変更に伴い
...
static int collect_monsters(int grp_cur, s16b mon_idx[], byte mode)
{
	...
	///sysdel 賞金首をモンスター一覧で表示させるための処理関連
 ...
 */
///mod131223
///item TVAL アイテムのTVALごとの簡易表記　下のobject_group_tval[]と順番が一致している必要があるはず
...
static int collect_objects(int grp_cur, int object_idx[], byte mode)
{
	...
	for (i = 0; i < max_k_idx; i++)
	{
		...
		//	if (TV_LIFE_BOOK <= k_ptr->tval && k_ptr->tval <= TV_HEX_BOOK)
			///mod160619 不等号逆だったので直したがどう変わるだろう
...
/* XTRA HACK RESLIST */
///item res ~9コマンド　保有アイテムの耐性一覧
///mod131229 ~9コマンド　アイテム耐性フラグ変更
static void do_cmd_knowledge_inven_aux(FILE *fff, object_type *o_ptr, int *j, byte tval, char *where)
{
	 ...
	 */
	///item TVAl SVAL 耐性一覧画面に表示するアイテムの種類を指定　ドラゴン鎧も追加しよう
	///mod131223 tval
...
static void do_cmd_knowledge_uniques(void)
{
	...
	/* Display the file contents */
#ifdef JP
	show_file(TRUE, file_name, "まだ倒していないユニーク・モンスター", 0, 0);
#else
 ...
 */
///sys item 武器経験値一覧を表示
///del131227 武器経験値はskill_expと統合した
#if 0
 ...
 * Display skill-exp
 */
///sys 技能経験値　武器経験値のページと統合してしまおう。
///mod131226 skill 技能と武器技能の統合によりフォーマット変更
/*:::memo:ここを変更したときdump_aux_skills()も同様に変更*/
static void do_cmd_knowledge_skill_exp(void)
{
	...
	for (i = 0; i < SKILL_EXP_MAX; i++)
	{
		if(i==5)
		{ 
			/*::: hack: p_ptr->skill_exp[5~9]は未使用なのでスキップ*/
			i=9;
			fprintf(fff, "\n");
			continue;
		}
...
static void do_cmd_knowledge_pets(void)
{
	...
	/* Display the file contents */
#ifdef JP
	show_file(TRUE, file_name, "現在の配下", 0, 0);
#else
...
static void do_cmd_knowledge_buildings(void)
{
	...
	//小傘にアイテムを預けている場合　詳細はkogasa_smith()参照
	if (inven_special[INVEN_SPECIAL_SLOT_KOGASA_SMITH].k_idx)
	{
		...
		if (need_turn > 0)
		{
			...
			fprintf(fff, "小傘に%sを預けている。\n約束の刻限まであと%02d:%02dだ。\n\n",o_name,h,m);

		}
		else
		{
			fprintf(fff, "小傘に%sを預けている。もう作業が済んでいるはずだ。\n\n", o_name);
		}
	...
	//草の根ネットワークでアイテム交換会中の場合　詳細はgrassroots_barter()参照
	if (p_ptr->grassroots > 1)
	{
		int need_turn;

		need_turn = (p_ptr->grassroots + 50000) - turn; //アイテムを渡したターンから半日(50000ターン)後
		...
		if (need_turn > 0)
		{
			...
			fprintf(fff, "草の根妖怪ネットワークにアイテムを預けている。\nアイテム交換会の開始まであと%02d:%02dだ。\n\n", h, m);
		}
		else
		{
			fprintf(fff, "草の根妖怪ネットワークでアイテム交換会が始まっている頃だ。神社に急ごう。\n\n");
		}
		flag_notice_sth = TRUE;
	}
	//霊夢の占い　詳細はreimu_fortune_teller()参照
	else if (p_ptr->today_mon)
	{
		...
		case FORTUNETELLER_TROPHY:
			...
			fprintf(fff, "霊夢の占いによると、今日中に%sを倒すと幸福が訪れるらしい。\n\n", (r_name + r_info[target_r_idx].name));
			flag_notice_sth = TRUE;
			break;
		case FORTUNETELLER_GOODLUCK:
			fprintf(fff, "霊夢の占いによると、今日の自分の運気は絶好調らしい！\n\n");
			flag_notice_sth = TRUE;
			break;
		case FORTUNETELLER_BADLUCK:
			fprintf(fff, "霊夢の占いによると、今日の自分の運気は絶不調らしい……\n\n");
			flag_notice_sth = TRUE;
			break;
		default:
	...
	//EXTRAモードでお燐にアイテムを配達してもらっている場合　詳細はorin_deliver_item()参照
	if (inven_special[INVEN_SPECIAL_SLOT_ORIN_DELIVER].k_idx)
	{
		...
		if (need_turn > 0)
		{
			...
			fprintf(fff, "火焔猫燐に%sを配達してもらっている。\nあと%02d:%02dくらいで着くはずだ。\n\n", o_name, h, m);

		}
		else
		{
			fprintf(fff, "火焔猫燐に%sを配達してもらっている。もう到着しているはずだ。\n\n", o_name);
		}
	...
	if(!flag_notice_sth)
		fprintf(fff, "特に心に留めておくことはない。\n\n");

	my_fclose(fff);
	show_file(TRUE, file_name, "現在の施設利用状況", 0, 0);
...
#ifdef JP
			///msg131221
			else c_put_str((r_ptr->max_num == 0 ? TERM_L_DARK : TERM_WHITE), (r_ptr->max_num == 0 ? "打倒済" : "未打倒"), row + i, 74);
#else
 ...
 */
/*:::今回の＠で判別済みのアイテムを一覧表示する*/
///pend 判別済みアイテム一覧　うまく表示されない
static void do_cmd_knowledge_objects(bool *need_redraw, bool visual_only, int direct_k_idx)
{
	int i, len, max;
	int grp_cur, grp_top, old_grp_cur;
...
static void do_cmd_knowledge_stat(void)
{
	...
	if (fff)
	{
		...
		*/
		...
		fprintf(fff, "あなたのスコア倍率 : %d%%\n", mult_race_class);

		//v1.1.79 現在使用中の移動屋敷を表示する
			...
			if (quest[i].type != QUEST_TYPE_RANDOM)
			{
				...
				if (quest[i].status == QUEST_STATUS_TAKEN || quest[i].status == QUEST_STATUS_STAGE_COMPLETED)
				{
					...
					case QUEST_TYPE_KILL_ANY_LEVEL:
						...
						else if (i == QUEST_DREAMDWELLER)
						{
							if(is_special_seikaku(SEIKAKU_SPECIAL_SUMIREKO))
								sprintf(note, " - あなたの悪夢を倒す。");
							else
								sprintf(note, " - 夢の世界のあなたを倒す。");
						}
						else
#ifdef JP
					...
					case QUEST_TYPE_FIND_RANDOM_ARTIFACT:
						if (quest[i].k_idx)
						{
							sprintf(note," - 伝説の%sを見つけ出す。", (k_name + k_info[quest[i].k_idx].name));
						}
						break;
					...
					case QUEST_TYPE_DELIVER_ITEM:
						...
						if(i == QUEST_KAGUYA)
						{
							///mod151219 全達成後に金閣寺追加
							if(kaguya_quest_level < KAGUYA_QUEST_MAX)
								sprintf(note,format(" - %sを探し出す。",a_name+a_info[kaguya_quest_art_list[kaguya_quest_level]].name));
							else
								sprintf(note,format(" - %sを探し出す。",k_name+k_info[lookup_kind(TV_SOUVENIR,SV_SOUVENIR_KINKAKUJI)].name));
						}
						else
							sprintf(note," - 指定されたアイテムを届ける。");
				...
				if (quest[i].status == QUEST_STATUS_COMPLETED)
				{
					...
					if(i == QUEST_MOON_VAULT || i == QUEST_MARICON)
						sprintf(tmp_str, "    クエスト達成 - まだ脱出していない。\n");
					else
...
void do_cmd_knowledge_quests_completed(FILE *fff, int quest_num[])
{
	...
	for (i = 1; i < max_quests; i++)
	{
		...
		if (quest[q_idx].status == QUEST_STATUS_FINISHED)
		{
			...
			else if(EXTRA_MODE && !is_fixed_quest_idx(q_idx)) //エクストラクエスト
			{
				int old_quest = p_ptr->inside_quest;
				p_ptr->inside_quest = q_idx;
				init_flags = INIT_NAME_ONLY;
			...
			else
			{
				/* Print the quest info */
#ifdef JP
				sprintf(tmp_str, "  %-40s (危険度:%3d階相当) - レベル%2d%s\n",
...
bool ang_sort_comp_quest_num(vptr u, vptr v, int a, int b)
{
	...
	//v1.1.69 同レベルなら達成時刻が早いほうが先に表示されるようにする(time値が記録されてないとき除く)
	...
	{
		//v1.1.70 同レベルでなくても達成時刻が早いほうが先に表示されるようにした。
		//達成時刻が記録されていない場合は低レベルなものから先に表示する
...
void do_cmd_time(void)
{
	...
	//v1.1.18 timenormとtimefunを書き換えた
#if 0
	/* Find the path */
